...
urlpatterns = [
    ...
    # 判断用户名是否重复注册
    re_path(
        r"^usernames/(?P<username>[a-zA-Z0-9_-]{5,20})/count/$",
    ...
    # 更新和删除地址
    re_path(
        r"^addresses/(?P<address_id>\d+)/$", views.UpdateDestroyAddressView.as_view()
    ),
    # 设置默认地址
    re_path(
        r"^addresses/(?P<address_id>\d+)/default/$", views.DefaultAddressView.as_view()
    ),
    # 更新地址标题
    re_path(
        r"^addresses/(?P<address_id>\d+)/title/$",
        views.UpdateTitleAddressView.as_view(),
    ),
    # 修改密码
    re_path(r"^change_pass/$", views.ChangePasswordView.as_view(), name="pass"),
    # 用户商品浏览记录
